...
void usb_goal_service();       // Non-blocking USB goal parser
...
void process_usb_line(const char* line) {
    ...
    // Clamp to Feetech ranges
    if (pos < 0)    pos = 0;
    if (pos > 4095) pos = 4095;
    if (speed < 0)    speed = 0;
    if (speed > 1023) speed = 1023;

    enqueue_goal_cmd(SERVO_ID, (uint16_t)pos, (uint16_t)speed);
}

void usb_goal_service() {
    // Hard cap to avoid spending too long on USB when burst arrives
    const int MAX_BYTES_PER_LOOP = 32;
    int processed = 0;
...
void setup() {
    ...
    Serial.println("USB goal streaming enabled: send 'G,<pos>,<speed>' commands");
...
void loop() {
    ...
    // ===== PRIORITY 2.5: USB Goal Parser (host commands) =====
    // Parses "G,<pos>,<speed>\n" commands from USB serial
    usb_goal_service();
