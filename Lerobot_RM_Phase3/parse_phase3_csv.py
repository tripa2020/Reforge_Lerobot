#!/usr/bin/env python3
"""
LeRobot Phase 3 (Option B') CSV Parser with Integrated Analysis
...
Phase 3 Architecture (Option B' - Synchronized TX):
...
Usage:
  ...
  python3 parse_phase3_csv.py /dev/ttyACM0 2000000 -o test.csv -v

  # Timed capture (auto-stops after 60 seconds, then analyzes)
  python3 parse_phase3_csv.py /dev/ttyACM0 2000000 -o test.csv --duration 60

  # Skip analysis
  python3 parse_phase3_csv.py /dev/ttyACM0 2000000 -o test.csv --no-analyze
...
# ============== PHASE 3 DATA STRUCTURE (64 bytes) ==============
# Must match SensorData struct in Lerobot_RM_Phase3.ino
# Phase 3: Option B' - Synchronized TX Architecture
...
def analyze_phase3(csv_file, save_plots=True):
    """
    Analyze Phase 3 (Option B') CSV data and print validation results.
    ...
    print("\n" + "=" * 70)
    print("LeRobot Phase 3 (Option B') Validation Analysis")
    print("=" * 70)
    ...
    print("-" * 70)
    print("NOTE: Phase 3 ISR includes IMU read + servo TX queue (~100µs expected)")
    ...
    # ========== OVERALL RESULT ==========
    print("\n" + "=" * 70)
    print("PHASE 3 (OPTION B') VALIDATION SUMMARY")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    if all_pass:
        print("✅ ALL TESTS PASSED - Phase 3 (Option B') Validated!")
...
def main():
    parser = argparse.ArgumentParser(
        description="LeRobot Phase 3 (Option B') CSV Parser with Integrated Analysis",
...
Examples:
  ...
  python3 parse_phase3_csv.py /dev/ttyACM0 2000000 -o test.csv --no-analyze
    ...
    if not args.no_analyze and sample_count > 0:
        analyze_phase3(args.output, save_plots=not args.no_plots)
